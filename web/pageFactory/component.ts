...
import { ComponentProps, LocatorProps } from '../types/pageFactory/component.type';
...
export abstract class Component {
  ...
  async shouldBeVisible(locatorProps: LocatorProps = {}): Promise<void> {
    await test.step(`${this.typeOfUpper} "${this.componentName}" should be visible on the page`,
      async () => {
  ...
  async click(locatorProps: LocatorProps = {}): Promise<void> {
    await test.step(`Кликнуть ${this.typeOf} with name "${this.componentName}"`, async () => {
      const locator = this.getLocator(locatorProps);
      await locator.click();
    });
  }
