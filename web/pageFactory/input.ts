...
import { LocatorProps } from '../types/pageFactory/component.type';
...
export class Input extends Component {
  ...
  async fill(value: string, fillProps: FillProps = {}): Promise<void> {
    ...
    await test.step(`Fill ${this.typeOf} "${this.componentName}" to value "${value}"`,
      async () => {
  ...
  async shouldHaveValue(value: string, locatorProps: LocatorProps = {}): Promise<void> {
    await test.step(`Checking that ${this.typeOf} "${this.componentName}" has a value "${value}"`,
      async () => {
        const locator = this.getLocator(locatorProps);
